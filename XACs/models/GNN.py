...
from XACs.models.chemprop import DMPNNEncoder
...
class GNN(torch.nn.Module):
    def __init__(
        ...
        if embed_method == 'ifp':
            ...
            self.edge_emb = torch.nn.Identity()
            self.node_hidden_dim = num_node_features
            self.edge_hidden_dim = num_edge_features
        ...
        for i in range(self.num_layers):
            if conv_name == "nn":
                conv = NNConv(
                    in_channels=self.node_hidden_dim if i == 0 else hidden_dim,
                    out_channels=hidden_dim,
                    nn=Sequential('edge_attr', [
                        (Lin(self.edge_hidden_dim, self.node_hidden_dim*hidden_dim if i == 0 else hidden_dim*hidden_dim), 'edge_attr -> edge_attr'),
                        ]),
                )
            elif conv_name == "gine":
                conv = GINEConv(Seq(Lin(self.node_hidden_dim if i == 0 else hidden_dim, 2*hidden_dim),
                                    ReLU(), 
            ...
            elif conv_name == "dmpnn":
                conv = DMPNNEncoder(hidden_dim, node_hidden_dim, edge_hidden_dim, depth=self.num_layers)
                self.convs.append(conv)
                break
            elif conv_name == "pna":
                conv = PNAConv(
                    in_channels=self.node_hidden_dim if i == 0 else 512//(2**(4-i)),
                    out_channels=512//(2**(3-i)),
                    edge_dim=self.edge_hidden_dim,
                    aggregators=['mean', 'min', 'max', 'sum', 'mul'],
            ...
            if conv_name == "gat":
                ...
                self.convs.append(Sequential('x, edge_index, edge_attr', [
                    ...
                    torch.nn.BatchNorm1d(512//(2**(3-i))), 
                    ReLU(),
                    #self.dropout
                    ]))
            else:
        ...
        if att_method == 'mlp':
            self.att_mlp = MLP([hidden_dim, hidden_dim//2, 1], dropout=dropout_rate, norm=None)
    ...
    def mlp_att_forward(self, batched_data, return_attention_weights=False):
        (batched_x, 
         batched_edge_index,
         batched_edge_attr,
         graph_mask,
         ...
         batched_batch) = (batched_data.x,
            ...
            batched_data.common_atom_mask,
            batched_data.batch)
        x = self.node_emb(batched_x)
        edge_attr = self.edge_emb(batched_edge_attr)
        atts = []
        for i in range(self.num_layers):
            if self.conv_name == "gat":
                if return_attention_weights:
                    x, att = self.convs[i](x, batched_edge_index, edge_attr, return_attention_weights=return_attention_weights) 
                    atts.append(att)
                else:
                    x = self.convs[i](x, batched_edge_index, edge_attr)
                x = x.relu()
            elif self.conv_name == "dmpnn":
                x = self.convs[i](x, batched_edge_index, edge_attr, mini_batch)
                break
            else:        
                x = self.convs[i](x, batched_edge_index, edge_attr)
        # get attribution through mlp
        att = self.att_mlp(x)
        if self.normalize_att:
            att = (att - att.min()) / (att.max() - att.min())
        uncom_att, common_att = att*uncom_mask, att*common_mask
